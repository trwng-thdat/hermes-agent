...
/** Every question of one clarify_form call, rendered as a single card the user submits once. */
export interface ClarifyFormRequestParams {
  session_id: string
  questions: ClarifyFormQuestion[]
}
/** One field of the batched clarify card. ``multiSelect`` keeps the tool's camelCase spelling. */
export interface ClarifyFormQuestion {
  id: string
  question: string
  choices?: string[] | null
  multiSelect?: boolean
}
/** ``answers`` maps question id → answer; a response without it is a cancel. */
export interface ClarifyFormResult {
  answers?: Record<string, string> | null
}
...
export interface ServerRequestMap {
  ...
  /** The clarify_form tool: one card carrying several questions, answered in one submit. */
  clarify_form: { params: ClarifyFormRequestParams; result: ClarifyFormResult }
